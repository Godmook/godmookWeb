...
def createBw():
    ...
    # Q2: Allow x and y to vary around the can at the same height,
    # only allow rotation about φ (yaw), fix z, ψ (roll), θ (pitch).
    Bw = np.zeros((6, 2))
    Bw[0, 0] = -0.1  # x_min
    Bw[0, 1] = 0.1   # x_max
    Bw[1, 0] = -0.1  # y_min
    Bw[1, 1] = 0.1   # y_max
    Bw[2, 0] = 0.0   # z_min (fixed)
    Bw[2, 1] = 0.0   # z_max (fixed)
    Bw[3, 0] = 0.0   # roll_min (fixed)
    Bw[3, 1] = 0.0   # roll_max (fixed)
    Bw[4, 0] = 0.0   # pitch_min (fixed)
    Bw[4, 1] = 0.0   # pitch_max (fixed)
    Bw[5, 0] = -np.pi  # yaw_min
    Bw[5, 1] = np.pi   # yaw_max
    ###
...
def main(if_sim):
    ...
    for configuration in configurations:
        ...
        # Since we're focusing on Q2, we'll leave this section unchanged.
        ###
    ...
    # We'll leave this section unchanged as per your instructions.
    ###
